...
export default class DeckGLOverlay extends Component {
  ...
  render() {
    const {viewport, data, isOutFlow, iconAtlas, iconMapping, iconData} = this.props;
    ...
    const iconScaling = Math.min(Math.pow(1.5, viewport.zoom - 10), 1);
    const updateTrigger = Math.floor(viewport.zoom);
    ...
    if (iconAtlas) {
      layers.push(
        new IconLayer({
          ...
          fp64: true,
          iconData,
          pickable: this.props.onIconHover,
          iconAtlas,
          iconMapping,
          sizeScale: ICON_SIZE * iconScaling * window.devicePixelRatio,
          getPosition: d => d.coordinates,
          getIcon: d => {
            return 'marker';
          },
          onHover: this.props.onIconHover,
          updateTriggers: {
